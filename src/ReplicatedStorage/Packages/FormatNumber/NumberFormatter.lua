	...
	return function(self, value)
		...
		if type_enum then
			if type(value) ~= "number"
				or math.floor(value) ~= value or value < 0 or value > typ then
...
local function round_fmt(fmt, fmt_n, intg_i, prec, rounding_mode)
	...
	if prec.type ~= "unlimited" then
		...
		elseif ro_i < fmt_n then
			intg_i_incr = 1
			midpoint_cmp = fmt[ro_i + 1] == 5
				and (ro_i + 1 == fmt_n and 0 or 1)
...
local function format_numberformatter(self, is_negt, fmt, fmt_n, intg_i)
	...
	elseif fmt == "inf" then
		...
		if notation.type == "scientific" then
			...
			if (notation.exponentsign == 0
				or notation.exponentsign == 4) and is_expt_negt
		...
		if resolved.grouping ~= 0
			and intg_w > 5 - resolved.grouping then
